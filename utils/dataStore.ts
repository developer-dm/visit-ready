...
// Combined User State
type UserState = {
    // App data (stored encrypted in AsyncStorage)
...
// App data store (encrypted in AsyncStorage)
export const useDataStore = create(
    persist<Pick<UserState, 'signup' | 'appointments' | '_dataHasHydrated' | 'addSignupData' | 'resetSignup' | 'addAppointment' | 'resetAppointments' | 'setDataHasHydrated'>>(
        ...
        }),
        {
            name: "data-store",
            storage: isWeb
                ? createJSONStorage(() => localStorage) // For web, use localStorage (you may want to add encryption for web too)
